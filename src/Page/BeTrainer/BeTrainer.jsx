...
const BeTrainer = () => {
 ...
 const [selectedDays, setSelectedDays] = useState([])
  ...
  const trainerInfo ={name,email,age,availableTimeDay,availableTimeWeek,skill,profileImage,trainerFrom,trainerStatus}
...
const handleDayChange = (selectedOptions) => {
  ...
  console.log(skills);
  const skillName = ['Functional Training','Strength Training','Pilates','Yoga','Dancing Workout','Pre and Post Natal Exercise']

  return (
    <div>
      <div className='border-black border md:w-[60%] dark:text-black p-10 bg-slate-200 my-20 mx-auto'>
        <h2 className='text-xl font-semibold text-center border-b-2 w-fit mx-auto px-4 border-black  border-dashed mb-5'>Give information to apply for trainer</h2>
      <form onSubmit={handleSubmit} className='max-w-md mx-auto'>
        <div className='mb-4'>
          <label className='block text-sm font-bold mb-2'>Full Name: (Read Only)</label>
          <input
            type='text'
            name='name'
        ...
        <div className='mb-4'>
          <label className='block text-sm font-bold mb-2'>Email: (Read Only)</label>
          <input
            type='email'
            name='email'
            value={user?.email}
        ...
        <div className='mb-4 '>
          ...
          <div className='flex flex-wrap gap-3'>
            {
              skillName.map((skill,index)=><div key={index} className='bg-slate-300 p-1 rounded-md '>
        ...
        <button
         ...
         {
    loading ? <div className='w-fit mx-auto '><Loader/> </div>
    : 'Be a Trainer'
